...
export const updatePost = (id: string, body: any) => {
  let foundPost = posts.findIndex((p) => p.id === Number(id));

  if (foundPost === undefined || foundPost === null)
    throw new Error("Post not found");

  posts[foundPost].id = body.id;
  posts[foundPost].category = body.category;
  posts[foundPost].content = body.content;
  posts[foundPost].title = body.title;
  posts[foundPost].image = body.image;
};
